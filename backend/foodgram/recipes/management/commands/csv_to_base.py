...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        # paths - словарь, где ключи - названия таблиц, а значения - пути до
        # *csv файлов с соответсвующими таблицами. 'static', 'data', здесь
        # типа /static/data/. То есть описывается путь
        # от рабочей папки dir_path до самого файла
        paths = {
            'recipes_ingredients': Path(
                dir_path, 'data', 'ingredients.csv'
            ),
        }
        ...
        # короче, забегая вперед надо сказать, что csv таблицы в
        # скрипте открываются два раза:
        # первый раз - для того, чтобы собрать специальные строки из
        # имен таблиц (из первой строки)
        # второй раз - для того, чтобы непосредственно скопировать
        # данные из csv в sqlite
        ...
        for table in tables:
            ...
            # открываются (первый раз) файлы для чтения с таблицей CSV,
            # сразу происходит
            ...
            with open(paths[table], 'r', encoding='utf-8') as wrapper:
                ...
                # ниже в цикле собираются строка fieldsline из имен
                # таблиц типа (col1, col2, col3)
                # и строка answer_sign_line типа (?, ?, ?, ?),
                # где количество ? равно
            ...
            with open(paths[table], 'r', encoding='utf-8') as wrapper:
                ...
                # migration_to_db - переменная,куда помещаются
                # данныеиз csv таблиц
                # она представляет собой список, который собирается из
                # кортежей и компрехеншена (цикла)
                # чтобы вот такой список получился
                # [(i['col1'], i['col2']) for i in reader],
                # где i == имя таблицы
        ...
        self.stdout.write(
            'Привет! Данные из *.csv теперь в базе!'
        )
